...
class CLIP(nn.Module):
	def __init__(self,
		...
		self.keep_ratio = nn.Parameter(torch.tensor(0.8))
		self.temperature = nn.Parameter(torch.tensor(0.7))

	def filter_patches(self, x: torch.Tensor) -> torch.Tensor:
		...
		similarity = torch.einsum('bpc,jpc->bjp', patches_norm, patches_norm)
		eye_mask = torch.eye(B, device=x.device).bool()
		similarity = similarity.masked_fill(eye_mask.unsqueeze(-1), 0)
		avg_similarity = similarity.sum(dim=1) / (B - 1)

		weights = F.softmax(avg_similarity / self.temperature, dim=-1)
		keep_num = max(1, int(self.keep_ratio * num_patches))

		oversample_factor = 3
		indices = torch.multinomial(
			weights,
			num_samples=keep_num * oversample_factor,
			replacement=True
		)  # [B, keep_num*3]

		uniform_indices = []
		for b in range(B):
			unique, _ = torch.unique(indices[b], return_inverse=True, sorted=False)

			# 强制对齐到 keep_num 长度
			if len(unique) >= keep_num:
				selected = unique[:keep_num]  # 直接截断
			else:
				# 不足时随机填充
				pad_num = keep_num - len(unique)
				pad = torch.randint(0, num_patches, (pad_num,), device=x.device)
				selected = torch.cat([unique, pad])

			uniform_indices.append(selected)

		# 转换为张量（确保形状一致）
		indices = torch.stack(uniform_indices)  # [B, keep_num]

		# 收集特征
		selected_patches = torch.gather(
			patches,
			dim=1,
			index=indices.unsqueeze(-1).expand(-1, -1, C)
		)

		return torch.cat([class_token, selected_patches], dim=1)
	...
	def encode_image(self, image):
		return self.filter_patches(self.visual(image.type(self.dtype)))
	#return self.visual(image.type(self.dtype))
